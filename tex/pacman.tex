...
L'utilisation d'un fully connected layer est permises car le problème est dans a situation suivante :
\begin{itemize}
...
Ces méthodes sont implémentées à l'aide de TensorFlow, sur une version pré-existante de PAC-MAN en Python (cf. Berkeley's Introduction to Artificial Intelligence).

\paragraph{Training set}

La base du training set est composée des images RGB brutes, de deux dispositions différentes :
\begin{itemize}
...
Ces images ont été récupérées avec \texttt{ImageGrab}. Un dataset de 6500 images étiquetées a été créé en jouant à PAC-MAN manuellement.	
Elles sont ensuite ré-échantillonnées avec \texttt{opencv} pour passer d'une résolution de 540x540 à 224x224 afin de pouvoir utiliser Inception V3 pour extraire les features.
